...
var (
	filter_Listings_UpdateListing_0 = &utilities.DoubleArray{Encoding: map[string]int{"listing": 0, "id": 1}, Base: []int{1, 2, 1, 0, 0}, Check: []int{0, 1, 2, 3, 2}}
)
...
func request_Listings_UpdateListing_0(ctx context.Context, marshaler runtime.Marshaler, client ListingsClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {
	...
	val, ok = pathParams["listing.id"]
	if !ok {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "missing parameter %s", "listing.id")
	}

	err = runtime.PopulateFieldFromPath(&protoReq, "listing.id", val)

	if err != nil {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "type mismatch, parameter: %s, error: %v", "listing.id", err)
	}
...
var (
	...
	pattern_Listings_GetListing_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 4, 1, 5, 2, 1, 0}, []string{"v1", "listings", "id"}, ""))

	pattern_Listings_CreateListing_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1}, []string{"v1", "listings"}, ""))

	pattern_Listings_UpdateListing_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 1, 0, 4, 2, 5, 2}, []string{"v1", "listings", "listing.id"}, ""))

	pattern_Listings_DeleteListing_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 1, 0, 4, 2, 5, 2}, []string{"v1", "listings", "id"}, ""))
)
